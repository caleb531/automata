...
class ManimNode(manim.VGroup):
    """
    The `ManimNode` class represents a state in FA as a node in `FAGraph` by representing
    the `Node` object in the `AGraph` object generated by `fa.show_diagram()` with
    manim.
    ...
    def __init__(self, node: pgv.Node):
        ...
        if node.attr["shape"] == "point":
            self.shape = manim.Dot(radius=radius)
            self.add(self.shape)
        elif node.attr["shape"].endswith("circle"):
            circle = manim.Circle(radius=radius)
            self.shape = (
                manim.VGroup(
                    circle,
                    manim.Circle().surround(circle, buffer_factor=0.8),
                )
...
@dataclass(eq=False)
class ManimEdge(manim.VGroup):
    """
    The `ManimEdge` class represents a transition in FA as an edge in `FAGraph` by
    representing the `Edge` object in the `AGraph` object generated by
    `fa.show_diagram()` with manim.
    ...
    def __init__(self, edge: pgv.Edge):
        r"""
        Parameters
        ----------
        ...
        if label_text := edge.attr["label"]:
            self.label = manim.Text(label_text, font_size=float(edge.attr["fontsize"]))
            x, y = (float(pt) / 72 for pt in edge.attr["lp"].split(","))
            self.label.set_x(x)
            self.label.set_y(y)
            self.add(self.label)
    ...
    def __parse_spline(edge_pos: str) -> manim.VGroup:
        ...
        control_points = tuple(
            (*(float(pt) / 72 for pt in point.split(",")), 0) for point in points[1:]
        )
        result = manim.VGroup()
        for i in range(0, len(control_points) - 1, 3):
            result.add(manim.CubicBezier(*control_points[i : i + 4]))
        endp = (*(float(pt) / 72 for pt in points[0].split(",")[1:]), 0)
        result.add(
            manim.Arrow(
...
class ManimInput(manim.VGroup):
    """
    The `ManimInput` class represents the input string. The characters of the input string
    is separated with each character is generated to a `Text` object, so that you can
    get each character simply with `[]` operator.
    """
    ...
    def show_result(self, accept: bool) -> manim.Write:
        ...
        """
        self.add(
            result := manim.Text(f"→ {'accept' if accept else 'reject'}").next_to(self)
        )
